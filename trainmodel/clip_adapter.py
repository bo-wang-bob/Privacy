...
DEFAULT_CLIP_ADAPTER_TEMPLATE = "a photo of a {class}."
OXFORDPETS_CLIP_ADAPTER_TEMPLATE = (
    "a photo of a {class}, a type of pet."
)


def get_clip_adapter_prompt_template(dataset_name: str | None) -> str:
    """Return the fixed zero-shot template used by CLIP-Adapter."""
    if dataset_name and dataset_name.lower() == "oxfordpets":
        return OXFORDPETS_CLIP_ADAPTER_TEMPLATE
    return DEFAULT_CLIP_ADAPTER_TEMPLATE


@torch.inference_mode()
def build_clip_adapter_text_features(
...
class CLIPBottleneckAdapter(nn.Module):
...
class CLIPAdapter(nn.Module):
    ...
    trainable_state_filename = "final_clip_adapter.pt"
    ...
    def __init__(
        ...
        if configured_dim != projection_dim:
            raise ValueError(
                "CLIP-Adapter feature_dim must match CLIP projection_dim "
        ...
        self.device = device
        self.adapter = CLIPBottleneckAdapter(
    ...
    def clone_adapter_only(self) -> "CLIPAdapter":
        clone = CLIPAdapter(
    ...
    def __deepcopy__(self, memo: dict) -> "CLIPAdapter":
        if id(self) in memo:
            return memo[id(self)]
