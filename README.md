...
每个 `run.log` 的第一行记录一次完整任务身份，包括模型、数据集、防御、run id、
阶段和 GPU；这些固定字段不会在后续每行重复。训练期间不输出逐通信轮次状态，只在
配置的正式评估轮次和最终轮输出 `Progress`，记录轮次、loss、accuracy、学习率、
参与客户端和审计累计数，并在 CoLA 等任务中同时记录 MCC。
